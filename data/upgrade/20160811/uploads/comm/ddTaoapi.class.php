...
class ddTaoapi extends Taoapi{
	...
	function do_rebate_order($data){
		...
		foreach($data as $k=>$row){
			...
			$trade=$duoduo->select('tradelist','id,status','trade_id_former="'.$row['trade_parent_id'].'" and app_key=0');
			if($trade['id']>0){
				if($trade['status']<5){
					$duoduo->update('tradelist',array('del'=>1,'trade_id'=>$row['trade_parent_id'].'del'),'trade_id_former="'.$row['trade_parent_id'].'" and app_key=0',0);
				}
				else{
					unset($data[$k]);
					continue;
