...
(function($) {
	$.fn.KinSlideshow = function(settings) {
		...
		function KSS_start() {
			...
			var w=$(ksbs + " a img").width();
			if(w>$(ksbs).width()){
				w=$(ksbs).width();
				$(ksbs + " a img").width(w);
			}
			KSS_Size.push(w);
			var h=$(ksbs + " a img").height();
			if(h>$(ksbs).height()){
				h=$(ksbs).height();
				$(ksbs + " a img").height(h);
			}
		...
		function KSS_setBtn() {
			...
			if (settings.isHasBtn && KSS_imgaeLength >= 2) {
				var html="<div class='KSS_btnBox' style='position:absolute;right:10px;bottom:5px; z-index:100'></div>";
				if(settings.closeBtn==true){
					html=html+"<div onclick='$(this).parent().hide()' style='position:absolute;right:10px;top:5px; z-index:100; color:#ffffff;font-size:20px;cursor:pointer'>×</div>";
				}
				$(ksthis).append(html);
				var KSS_btnList = "";
